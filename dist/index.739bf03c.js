...
},{"./utils":"72Dku","./panel":"cKLyM","./cursor":"3v1v0","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"72Dku":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
class Panel {
    ...
    layout() {
        ...
        this.marginImages = parseInt(getComputedStyle(document.body).getPropertyValue("--margin-images"), 2);
    ...
    openItem(item1) {
        if (this.isOpen) return;
        this.isOpen = true;
        this.currentItemIdx = this.items.indexOf(item1);
        //item.killActiveTimeline();
        // all other items (the .imgWrap element)
        const otherImgWrap = this.items.except(item1).map((item)=>item.DOM.imgWrap);
        (0, _gsap.gsap).timeline({
            onStart: ()=>item1.DOM.el.classList.add("panel__item--open")
    ...
    closeItem(item2) {
        ...
        (0, _gsap.gsap).timeline({
            onComplete: ()=>{
                item2.DOM.el.classList.remove("panel__item--open");
                this.isOpen = false;
            }
        }).addLabel("start", 0).to(item2.DOM.closeCtrl, {
            duration: 0.7,
            ease: "power3",
            opacity: 0,
            scale: 0.5
        }, "start").to(item2.DOM.titleTexts, {
            duration: 0.7,
            ease: "power4.inOut",
            opacity: 0,
            x: (i)=>i ? 100 : -100,
            rotation: 0.001
        }, "start").to(item2.DOM.subtitleHeader, {
            duration: 0.7,
            ease: "power4.inOut",
            opacity: 0,
            x: 20
        }, "start").to(item2.DOM.imgWrap, {
            duration: 0.7,
            ease: "power4.inOut",
            x: (_, t)=>this.calcImgPosition(t).tx
        }, "start").to(this.items.except(item2).map((item)=>item.DOM.imgWrap), {
            duration: 0.7,
            ease: "power4.inOut",
            startAt: {
                x: "+=20%"
            ...
            x: (_, t)=>this.calcImgPosition(t).tx,
            opacity: 1
        }, "start").to(item2.DOM.subtitleChars, {
            duration: 0.4,
            ease: "power4.inOut",
...
},{"gsap":"fPSuC","./utils":"72Dku","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["cVgJb","ebWYT"], "ebWYT", "parcelRequire94c2")
